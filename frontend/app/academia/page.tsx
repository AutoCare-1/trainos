...
const STATUS_ESTILO: Record<string, string> = {
  ...
  rejected: 'bg-ink/6 text-ink-muted',
}
  ...
  return (
    <>
      ...
      <main className="mx-auto w-full max-w-3xl flex-1 px-4 py-8">
        ...
        <div className="mb-6">
          <h1 className="mb-1 font-display text-2xl font-bold tracking-tight text-ink">Análises de academia</h1>
        ...
        {submissions === null && !erro && <p className="text-ink-muted">Carregando...</p>}

        {submissions?.length === 0 && (
          <div className="glass rounded-2xl border-dashed p-10 text-center">
            <p className="text-ink-muted">Nenhuma análise enviada ainda.</p>
            <p className="mt-1 text-sm text-ink-muted">Assim que um aluno enviar fotos ou vídeo da academia, aparece aqui.</p>
          </div>
        )}
          ...
          {submissions?.map((s) => (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate font-semibold text-ink">{s.student_name}</p>
